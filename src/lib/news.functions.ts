...
async function curateWithClaude(
  ...
  { index: number; region: FeedArticle["region"]; implication: string }[] | null
> {
  ...
  try {
    ...
    return parsed
      ...
      .map((p) => {
        ...
        return { index: p.index, region, implication };
      })
      .slice(0, 10);
  } catch (err) {
